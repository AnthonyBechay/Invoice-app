...
import { collection, query, where, onSnapshot, addDoc, updateDoc, deleteDoc, doc, serverTimestamp, orderBy, limit as firestoreLimit } from 'firebase/firestore';
...
const ExpensesPage = () => {
    ...
    const initializeDefaultCategories = async () => {
        ...
        try {
            const batch = DEFAULT_CATEGORIES.map(cat =>
                addDoc(collection(db, `expenses/${auth.currentUser.uid}/categories`), {
                    name: cat.name,
                    color: cat.color,
    ...
    const handleAddExpense = async () => {
        if (!auth.currentUser || !expenseDescription.trim() || !expenseAmount || !expenseCategory) {
    ...
    const handleEditExpense = (expense) => {
        ...
        setExpenseDate(expense.date.toDate().toISOString().split('T')[0]);
